...
- **v2 client foundation** ([#3](https://github.com/blksheep80/ex_pipedrive/issues/3)): `ExPipedrive.Client` owns base URL/`api_domain`; `ExPipedrive.Request` owns versioned paths (default `/api/v2`, explicit `:v1` fallback). Inherited resources still call v1 via `api_version: :v1` until #8/#9.
- Branch `feat/v2-client-foundation` (bead `expd-98l`) — not yet on `main`.
...
5. ~~#3 v2-first client foundation~~ (done on `feat/v2-client-foundation`)
