defmodule ExPipedrive.ProductFields do
  @moduledoc """
  API v2 product field definitions.

  `field_code` is the hash used as a key in `%ExPipedrive.Product{}.custom_fields`;
  ...
  @doc """
  Lists one cursor page of product field definitions via `GET /api/v2/productFields`.
  ...
  def list_page(%Client{} = client, opts \\ []) do
    ...
    client
    |> Request.get("productFields", query: query)
  ...
  @doc """
  Lazily streams product field definitions across API v2 cursor pages.
  """
  ...
  """
  @spec list_product_fields(Client.t(), keyword()) ::
          {:ok, Page.t()} | {:error, ExPipedrive.Error.t()}
  def list_product_fields(%Client{} = client, opts \\ []), do: list_page(client, opts)
end
