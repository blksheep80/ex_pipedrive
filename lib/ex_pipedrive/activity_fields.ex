defmodule ExPipedrive.ActivityFields do
  @moduledoc """
  API v2 activity field definitions.

  `field_code` is the hash used as a key in `%ExPipedrive.Activity{}.custom_fields`;
  ...
  @doc """
  Lists one cursor page of activity field definitions via `GET /api/v2/activityFields`.
  ...
  def list_page(%Client{} = client, opts \\ []) do
    ...
    client
    |> Request.get("activityFields", query: query)
  ...
  @doc """
  Lazily streams activity field definitions across API v2 cursor pages.
  """
  ...
  """
  @spec list_activity_fields(Client.t(), keyword()) ::
          {:ok, Page.t()} | {:error, ExPipedrive.Error.t()}
  def list_activity_fields(%Client{} = client, opts \\ []), do: list_page(client, opts)
end
