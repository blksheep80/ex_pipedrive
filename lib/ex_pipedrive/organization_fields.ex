defmodule ExPipedrive.OrganizationFields do
  ...
  `field_code` is the hash used as a key in
  `%ExPipedrive.Organization{}.custom_fields`; `field_name` is its
  ...
  @doc """
  Lists one cursor page of organization field definitions via
  ...
  def list_page(%Client{} = client, opts \\ []) do
    ...
    client
    |> Request.get("organizationFields", query: query)
  ...
  @doc """
  Lazily streams organization field definitions across API v2 cursor pages.
  """
  ...
  """
  @spec list_organization_fields(Client.t(), keyword()) ::
          {:ok, Page.t()} | {:error, ExPipedrive.Error.t()}
  def list_organization_fields(%Client{} = client, opts \\ []), do: list_page(client, opts)
end
