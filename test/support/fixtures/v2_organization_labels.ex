defmodule ExPipedrive.Fixtures.V2OrganizationLabels do
  @moduledoc false
  ...
  def field_response do
    %{
      ...
      "data" => %{
        ...
        "field_type" => "set",
        "options" => [
          option(1, "Partner", "blue"),
          option(2, "Vendor", "gray")
        ]
      },
      "additional_data" => nil
    }
    ...
    %{
      ...
      "data" =>
        Enum.map(options, fn %{"label" => label} = attrs ->
          option(99, label, Map.get(attrs, "color", "blue"))
        end),
      "additional_data" => nil
    }
  end
    ...
    %{
      ...
      "data" =>
        Enum.map(options, fn %{"id" => id} = attrs ->
          option(id, Map.get(attrs, "label", "Partner"), Map.get(attrs, "color", "blue"))
        end),
      "additional_data" => nil
    }
  end
  ...
  def delete_options_response(options) when is_list(options) do
    %{
      "success" => true,
      "data" => Enum.map(options, fn %{"id" => id} -> option(id, "Partner", "blue") end),
      "additional_data" => nil
    }
  end
